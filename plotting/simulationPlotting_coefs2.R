...
plotVals <- function(df, B, frac, fwer = NA) {
  ...
  plot(jitter(as.numeric(rownames(df)), amount = 0.1), df[,1], type = "o", col = mypal[1], ylim = c(0, 1), xlim = c(0, 7.5),
       xlab = "Coefficient (\u03B2)", ylab = "Power (-) and FWER (o)", main = paste0("Power with B = ", B, " Selection Frac = ", frac), 
       lty = 1, pch = (1 + 14), lwd = 3, cex.lab = 1.5, cex.axis = 1.5)
  for (col in 2:ncol(df)) {
    lines(jitter(as.numeric(rownames(df)), amount = 0.1), df[,col], type = "b", col = mypal[col], lty = col, pch = (col + 14),
          lwd = 3)
  }
  # if (!is.na(fwer)) {
...
simDf <- read_excel('simulations.xlsx')[c(6:9),]
...
mainFunc <- function() {
  ...
  rownames(powerMat) <- as.numeric(simDf$Coefficient)
  rownames(fwerMat) <- as.numeric(simDf$Coefficient)
