...
plotVals <- function(df, fwer = NA) {
  ...
  legend(0.5, 0.9, legend=round(as.numeric(legendNames), 2), col = mypal, lty = 1:4, pch = 15:18, lwd = 3, ncol = 2, cex = 2)
  # dev.off()
}
...
mainFunc <- function() {
  ...
  npVec <- c(as.numeric(simDf$n) / as.numeric(simDf$p))
  powerMat <- matrix(NA, nrow = length(frac.vec), ncol = length(npVec))
  fwerMat <- matrix(NA, nrow = length(frac.vec), ncol = length(npVec))
  ...
  powerMat <- powerMat[, order(as.numeric(colnames(powerMat)))]
  fwerMat <- fwerMat[, order(as.numeric(colnames(fwerMat)))]
  plotVals(powerMat, fwerMat)
}
