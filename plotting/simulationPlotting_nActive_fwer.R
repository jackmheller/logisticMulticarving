...
plotVals <- function(df, B, frac, fwer = NA) {
  ...
  plot(as.numeric(rownames(fwer)), fwer[,1], type = "o", col = mypal[1], ylim = c(0, 1), xlim = c(0.5, 1),
       xlab = "Selection Fraction", ylab = "FWER", main = expression(paste("FWER of Carving (", gamma, " = 0.05) with B = 10")), 
       lty = 1, pch = (1 + 14), lwd = 3, cex.lab = 1.5, cex.axis = 2, cex.main = 2)
  for (col in 2:ncol(fwer)) {
    lines(as.numeric(rownames(fwer)), fwer[,col], type = "b", col = mypal[col], lty = col, pch = (col + 14),
          lwd = 3)
  }
  # if (!is.na(fwer)) {
