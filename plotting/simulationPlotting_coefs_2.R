...
plotVals <- function(df, B, frac, fwer = NA) {
  legendNames <- c(expression(paste(beta, " = 1")), expression(paste(beta, " = 5")), expression(paste(beta, " = 2")),
                   expression(paste(beta, " = 7.5")), expression(paste(beta, " = 0.5")))
  ...
  legend('topleft', legend=legendNames, col = mypal, lty = 1:4, pch = 1:4 + 14, ncol = 3, cex = 2)
  # dev.off()
}
