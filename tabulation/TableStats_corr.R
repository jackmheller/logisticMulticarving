    ...
    for (frac in frac.vec) {
      for (row in 1:nrow(simDf)) {
        ...
        if (B == 1) {
          ...
          rownames(curMat) <- c(paste0('carve_B=', B, 'select=', frac, 'corr=', simDf[row, 13]), 
                                paste0('split_B=', B, 'select=', frac, 'corr=', simDf[row, 13]))
          ...
          rownames(curMat) <- c(paste0('carve_gam5_B=', B, 'select=', frac, 'corr=', simDf[row, 13]), 
                                paste0('split_gam5_B=', B, 'select=', frac, 'corr=', simDf[row, 13]),
                                paste0('carve_gam30_B=', B, 'select=', frac, 'corr=', simDf[row, 13]), 
                                paste0('split_gam30_B=', B, 'select=', frac, 'corr=', simDf[row, 13]))
          # load power
          curMat[1, 5] <- power[2]
          curMat[2, 5] <- power[4]
          curMat[3, 5] <- power[6]
