...
simDf$Time <- round(as.numeric(simDf$Time), 2)
  ...
  for (B in B.vec) {
    for (frac in frac.vec) {
      for (row in 1:nrow(simDf)) {
        ...
        if (B == 1) {
          retList <- calculatePowerB1(curDat, as.numeric(simDf[row, 12]))
        }
        else {
        ...
        if (B == 1) {
          ...
          rownames(curMat) <- c(paste0('carve_B=', B, 'select=', frac, 'nActive=', simDf[row, 12]), 
                                paste0('split_B=', B, 'select=', frac, 'nActive=', simDf[row, 12]))
          ...
          rownames(curMat) <- c(paste0('carve_gam5_B=', B, 'select=', frac, 'nActive=', simDf[row, 12]), 
                                paste0('split_gam5_B=', B, 'select=', frac, 'nActive=', simDf[row, 12]),
                                paste0('carve_gam30_B=', B, 'select=', frac, 'nActive=', simDf[row, 12]), 
                                paste0('split_gam30_B=', B, 'select=', frac, 'nActive=', simDf[row, 12]))
          # load power
          curMat[1, 5] <- power[2]
          curMat[2, 5] <- power[4]
          curMat[3, 5] <- power[6]
        ...
        if (B == 1) {
          expectations <- calculateExpectationsB1(curDat, as.numeric(simDf[row, 12]))
        }
        else {
          # capture EV and ERV
          expectations <- calculateExpectations(curDat, as.numeric(simDf[row, 12]))
        }
        EV <- expectations[[1]]
        ERV <- expectations[[2]]
        FDR <- expectations[[3]]
