...
simDf <- read_excel('simulations.xlsx')[c(1, 3, 22),]
    ...
    for (frac in frac.vec) {
      for (row in 1:nrow(simDf)) {
        ...
        if (B == 1) {
          ...
          rownames(curMat) <- c(paste0('carve_B=', B, 'select=', frac, 'link=', simDf[row, 14]), 
                                paste0('split_B=', B, 'select=', frac, 'link=', simDf[row, 14]))
          ...
          rownames(curMat) <- c(paste0('carve_gam5_B=', B, 'select=', frac, 'link=', simDf[row, 14]), 
                                paste0('split_gam5_B=', B, 'select=', frac, 'link=', simDf[row, 14]),
                                paste0('carve_gam30_B=', B, 'select=', frac, 'link=', simDf[row, 14]), 
                                paste0('split_gam30_B=', B, 'select=', frac, 'link=', simDf[row, 14]))
          # load power
          curMat[1, 5] <- power[2]
          curMat[2, 5] <- power[4]
          curMat[3, 5] <- power[6]
